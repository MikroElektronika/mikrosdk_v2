...
static void gridnav_event_cb(lv_event_t * e)
{
    ...
    if(_code == LV_EVENT_KEY) {
    ...
    else if(_code == LV_EVENT_CHILD_CREATED) {
        ...
        lv_gridnav_remove(obj);
            ...
            _code == LV_EVENT_SHORT_CLICKED || _code == LV_EVENT_LONG_PRESSED || _code == LV_EVENT_LONG_PRESSED_REPEAT ||
            _code == LV_EVENT_CLICKED || _code == LV_EVENT_RELEASED) {
        if(lv_group_get_focused(lv_obj_get_group(obj)) == obj) {
            ...
            if(t == LV_INDEV_TYPE_ENCODER || t == LV_INDEV_TYPE_KEYPAD) {
                lv_obj_send_event(dsc->focused_obj, _code, lv_indev_active());
            }
        }
    }
}
