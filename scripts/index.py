...
def index_release_to_elasticsearch(es : Elasticsearch, index_name, release_details, token, keep_previous_date=False, lvgl_only=False):
    necto_versions = {
    ...
    with alive_bar(len(release_details[0]['assets']), title='Indexing Packages', length=60, spinner='wait') as bar:
        for asset in release_details[0].get('assets', []):
            ...
            # If it was requested only to index LVGL - skip all other packages
            if lvgl_only and not name_without_extension.startswith('lvgl_'):
                continue
...
if __name__ == '__main__':
    ...
    parser.add_argument("--lvgl_only", help="Indexes only templates.", type=str2bool, default=False)
