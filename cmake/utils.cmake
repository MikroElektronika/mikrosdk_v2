...
function(find_chip_vendor _chip_vendor)
    ...
    if(${MCU_NAME} MATCHES "^(AT)?SAM.*")
        set(${_chip_vendor} "sam" PARENT_SCOPE)
    endif()
endfunction()
...
function(mikrosdk_get_log_libs listArg listSrcArg logInterface)
    ...
    if (DEFINED LOG_INTERFACE)
        if(${LOG_INTERFACE} STREQUAL "LOG_INTERFACE_USB")
            list(APPEND local_include_list MikroSDK.USB.Tiny)
            set(${logInterface} "usb" PARENT_SCOPE)
            list(APPEND local_file_list src/usb/log.c)
            list(APPEND local_file_list src/usb/log_printf_implementation.c)
            list(APPEND local_file_list src/usb/log_printf_implementation.h)
            list(APPEND local_file_list src/usb/usb_desrciptors.c)
            list(APPEND local_file_list src/usb/systick.h)
            list(APPEND local_file_list include/usb/log.h)
        elseif(${LOG_INTERFACE} STREQUAL "LOG_INTERFACE_UART")
