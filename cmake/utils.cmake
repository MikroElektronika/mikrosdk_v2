...
function(find_chip_architecture _chip_architecture)
    ...
    elseif(${CORE_NAME} MATCHES "RL78")
        set(${_chip_architecture} "rl78" PARENT_SCOPE)
    else()
...
function(set_resolution cmake_adc_resolution)
    ...
    elseif(${chip_architecture} STREQUAL "pic_16bit")
        set(${cmake_adc_resolution} "RESOLUTION_12_BIT" PARENT_SCOPE)
    elseif(${chip_architecture} STREQUAL "rl78")
        set(${cmake_adc_resolution} "RESOLUTION_12_BIT" PARENT_SCOPE)
    else()
...
function(core_implementation_select mcu core_implementation)
    find_chip_architecture(chip_architecture)
    if(${chip_architecture} STREQUAL "arm")
        if(${mcu} MATCHES "(^STM32.*)|(^MK.*)|(^TM4C.*)|(^(AT)?SAM.*)|(^R7.+)|(^TMPM.+)|(^M2354.+)")
        ...
        if (${mcu} MATCHES "(^AT.*)|(^PIC18.*)|(^PIC32.*)|(^(ds)?PIC(24|30|33).*)|(^GD32VF.*)|(^R7F100.*)")
